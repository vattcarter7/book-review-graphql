...
insert into br.author(id, name) values
  (1, 'David Kadavy'),
  (2, 'Peter Thiel'),
  (3, 'Blake Masters');

-- update tokens column for full text search
update br.author set tokens = to_tsvector(name);

-- reset next sequence used for id column.  Next id should be 4
-- since 1, 2 & 3 were used above
alter sequence br.author_id_seq restart with 4;

insert into br.book(id, google_id, title, subtitle, description, page_count, rating_total, rating_count, rating ) values
...
-- update tokens column for full text search
update br.book set tokens = to_tsvector(coalesce(title, '') || coalesce(subtitle, '') || coalesce(description, ''));

-- reset next sequence used for id column.
alter sequence br.book_id_seq restart with 3;

insert into br.book_author(id, book_id, author_id) values
  (1, 1, 2),
  (2, 1, 3),
  (3, 2, 1);

-- reset next sequence used for id column.
alter sequence br.book_author_id_seq restart with 4;

insert into br.user(id, email, password, name ) values 
  (1, 'carter@gmail.com', 'pass12345', 'carter'),
  (2, 'sopheak@gmail.com', 'pass12345', 'sopheak'),
  (3, 'david@gmail.com', 'pass12345', 'david');


-- update tokens column for full text search
update br.user set tokens = to_tsvector(name);

-- reset next sequence used for id column.
alter sequence br.user_id_seq restart with 4;

insert into br.review(id, user_id, book_id, rating, title, comment) values 
...
-- update tokens column for full text search
update br.review set tokens = to_tsvector(coalesce(title, '') || coalesce(comment, ''));

-- reset next sequence used for id column.
alter sequence br.review_id_seq restart with 7;
